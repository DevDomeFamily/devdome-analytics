...
/** Per-site cleanup: every site of a network gets it (DeepSeek round 4), the opt-in is read per site. */
function devdalyt_uninstall_site() {
	...
	if ( is_array( $devdalyt_fp ) && ! empty( $devdalyt_fp['dir'] ) && preg_match( '/^[a-f0-9]{10}$/', (string) $devdalyt_fp['dir'] ) ) {
		...
		$devdalyt_botd = trailingslashit( $devdalyt_dir ) . 'botd.js'; // the bot-detector copy next to the tracker (1.1.0)
		if ( $devdalyt_inside && ! is_link( $devdalyt_dir ) && ! is_link( $devdalyt_botd ) && is_file( $devdalyt_botd ) ) {
			wp_delete_file( $devdalyt_botd );
		}
	...
	delete_option( 'devdalyt_init_pending' ); // fresh-defaults marker (round 3)
...
// Per blog (Codex round 5): $wpdb->options follows switch_to_blog(), so the sweep and the shared-core
// cleanup run INSIDE each site's context on a network, not once on the main site after the loop.
function devdalyt_uninstall_sweep() {
	global $wpdb;
	$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->options} WHERE option_name LIKE %s OR option_name LIKE %s", $wpdb->esc_like( '_transient_devdalyt_' ) . '%', $wpdb->esc_like( '_transient_timeout_devdalyt_' ) . '%' ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery -- uninstall-time cleanup of dynamic transient keys
}
...
if ( is_multisite() ) {
	foreach ( get_sites( array( 'fields' => 'ids', 'number' => 0 ) ) as $devdalyt_blog_id ) {
		switch_to_blog( (int) $devdalyt_blog_id );
		devdalyt_uninstall_site();
		devdalyt_uninstall_sweep();
